...
public class ComplexOomage implements Oomage {
    ...
    public int hashCode() {
        int total = 0;
        for (int x : params) {
            total = total * 256; // 256 = 2^8即8位得移动, 而 > 256^4 in java is 0! 容易numeric overflow, 4-bit hashcode的冲突太大！
            // change to 255/257 may avoid the overflow (2^n mod 2^32 = 0 for n > 32) problem
            total = total + x;
        }
        return total;
    }
